...
void Output::inputFunction(double a, double e1){
    ...
    std::vector<double> comp = {a, e1, floor(exp(a)), ceil(exp(a)),  bis, false_pos, newton};
...
void Output::comparativeTable(){ // Printa
   ...
   std::cout << "------------------------------------------------------------------------" << std::endl;
     ...
     for (int i = 0; i < comparativeData.size(); i++) {
        ...
        std::setw(12) << comparativeData[i][4] << std::setw(17)
        << comparativeData[i][5] << comparativeData[i][6] <<std::endl; 
    }
}
